let collect = {
    ...
    async removeCollectionRelation(input, libraries, ctx) {
        let { PlatformApi } = libraries
        let iaf_collections = await PlatformApi.IafScriptEngine.getVar('iaf_asset_collection')
        let collectionType = iaf_collections
        console.log("removeCollectionRelation")
        let relatedItems = await PlatformApi.IafScriptEngine.attachItemsAsRelated({
            "parentItems": input.removeFrom,
            "relatedItems": input.removeEntities
        }, ctx)
        let removeResult
        if (collectionType) {
            removeResult = await PlatformApi.IafScriptEngine.removeRelations({
                "parentUserItemId": iaf_collections._userItemId,
                "_userItemId": collectionType._userItemId,
                "relations": relatedItems
            }, ctx)
        }
        let finalRemoveResult
        console.log(removeResult[0])
        if (removeResult[0][0] == "ok: 204") {
            finalRemoveResult = {
                "success": true
            }
        }
            ...
            finalRemoveResult = {
                "success": false,
                "message": "Error removing relations!"
            }
        }
        return finalRemoveResult
    },
}
